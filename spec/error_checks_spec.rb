...
RSpec.describe Checks do
  ...
  describe "#check_structure(file)" do
    it "returns error statement" do
      expect(check_structure("<img src=\"assets/logo.png\">")).to eql("poor structure check your #{"<html lang=\"en\">"} tag")
    end
  end
  ...
  describe "#check_head_tag_contents(file)" do
    it "returns error statement" do
      expect(check_head_tag_contents("<img src=\"assets/logo.png\">")).to eql("place #{"<title>"} in between <head></head>")
    end
  end
